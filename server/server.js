...
app.get("/brands/count", (req, res) => {
    db.collection("brands").countDocuments({}).then(result => res.json(result));
});
...
app.get("/models/count", (req, res) => {
    db.collection("models").countDocuments({}).then(result => res.json(result));
});
...
app.get("/models/:id", (req, res) => {
    if (ObjectId.isValid(req.params.id)) {
        ...
        .catch(err => res.status(500).json({ err: "Could not find the model" }));
    } else {
...
app.post("/models", (req, res) => {
    ...
    db.collection("models").insertOne(model).then(result=> {
...
app.patch("/models/:id", (req, res) => {
    ...
    if (ObjectId.isValid(req.params.id)) {
        ...
        .then(result => res.status(200).json(result))
        .catch(err => res.status(500).json({ err: "Could not update the model" }));
    } else {
        res.status(500).json({ err: "Invalid document id" });
    }
});
...
app.delete("/models/:id", (req, res) => {
    if (ObjectId.isValid(req.params.id)) {
        ...
        .then(result => res.status(200).json(result))
        .catch(err => res.status(500).json({ err: "Could not delete the model" }));
    } else {
        res.status(500).json({ err: "Invalid document id" });
    }
});